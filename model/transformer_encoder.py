...
class VAETransformerEncoder(nn.Module):
  def __init__(self, n_layer, n_head, d_model, d_ff, d_vae_latent, embedding_dim, num_embedding_dim, dropout=0.1, activation='relu'):
    ...
    self.fc_mu = nn.Linear(d_model, embedding_dim)
    self.fc_logvar = nn.Linear(d_model, num_embedding_dim)
